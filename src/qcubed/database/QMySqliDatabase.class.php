...
class QMySqliDatabase extends QDatabaseBase
{
    ...
    public function InsertOrUpdate($strTable, $mixColumnsAndValuesArray, $strPKNames = null)
    {
        ...
        $strSql = sprintf('INSERT INTO %s%s%s (%s) VALUES (%s) ON DUPLICATE KEY UPDATE %s', $this->EscapeIdentifierBegin, $strTable, $this->EscapeIdentifierEnd, implode(', ', array_keys($strEscapedArray)), implode(', ', array_values($strEscapedArray)), $strUpdateStatement
        );
    ...
    protected function ExecuteNonQuery($strNonQuery)
    {
        ...
        QApplication::Log(__FILE__, __FUNCTION__, $strNonQuery);
//      if($this->blnEnableProfiling) QFirebug::info($strNonQuery,__CLASS__."::".__FUNCTION__);
        // Perform the Query
    ...
    public function Connect()
    {
        ...
        $logdata = array("info" => "MySQLi Database Connect", "server" => $this->Server, "username" => $this->Username, "password" => $this->Password, "database" => $this->Database, "port" => $this->Port);
        QApplication::Log(__FILE__, __FUNCTION__, $logdata);
//      if($this->blnEnableProfiling) QFirebug::info($logdata,__CLASS__."::".__FUNCTION__);
    ...
    {
        performance(1, __CLASS__, __FUNCTION__);
        QApplication::Log(__FILE__, __FUNCTION__, $strQuery);
//      if($this->blnEnableProfiling) QFirebug::info($strQuery,__CLASS__."::".__FUNCTION__);
        // Perform the Query
                ...
                if ($objRow["Field"] == $this->strOriginalName) {
                    ...
                    if ((count($strLengthArray) > 1) &&
                        ...
                        if ($intCommaPosition !== false) {
                            $this->intDecimal = substr($this->intMaxLength, $intCommaPosition + 1);
