...
class QOracleDatabase extends QDatabaseBase
{
    ...
    public function Connect()
    {
        ...
        if (!$this->objOracle)
            throw new QOracleDatabaseException("Unable to connect to Database (Oracle Database Server might be down or wrong connection credentials)", -1, null);
        ...
        $this->NonQuery("ALTER SESSION SET NLS_SORT=BINARY_CI");
        $this->NonQuery("ALTER SESSION SET NLS_COMP=LINGUISTIC");
    }
    ...
    protected function Execute($objResult)
    {
        ...
        if ($objResult) {
            if ($this->commitMode) {
                $blnReturn = oci_execute($objResult, OCI_COMMIT_ON_SUCCESS);
            } else {
                $blnReturn = @oci_execute($objResult, OCI_COMMIT_ON_SUCCESS);
                if (!$blnReturn) {
    ...
    protected function ExecuteQuery($strQuery)
    {
        ...
        // Perform the Query
        //print $strQuery; die();
        $strQuery = str_replace(" __", " zz__", $strQuery);
        $objResult = oci_parse($this->objOracle, $strQuery);

        if ($objResult) {
            if ($this->commitMode) {
                oci_execute($objResult, OCI_COMMIT_ON_SUCCESS);
            } else {
                oci_execute($objResult, OCI_COMMIT_ON_SUCCESS);
            }
    ...
    protected function prep($strQuery)
    {
        // query format mysql menggunakan tick, yang ga fungsi di oracle
    ...
    protected function ExecuteNonQuery($strNonQuery)
    {
        ...
        // preprocessing query
        $strNonQuery = $this->prep($strNonQuery);
...
class QOracleDatabaseRow extends QDatabaseRowBase
{
    ...
    {
        foreach ($strColumnArray as $key => $val) {
            $key = str_replace("zz__", "__", strtolower($key));
            $this->strColumnArray[$key] = $val;
        }
    }

    public function GetColumn($strColumnName, $strColumnType = null)
    {
        if (array_key_exists($strColumnName, $this->strColumnArray) || array_key_exists(strtoupper($strColumnName), $this->strColumnArray)) {
            $strColumnValue = $this->strColumnArray[$strColumnName];
            if (!$strColumnValue) {
                $strColumnValue = $this->strColumnArray[strtoupper($strColumnName)];
            }
            ...
            switch ($strColumnType) {
                ...
                case QDatabaseFieldType::Blob:
                    if (is_object($strColumnValue) && method_exists($strColumnValue, "load")) {
