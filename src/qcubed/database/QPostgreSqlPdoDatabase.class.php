...
class QPostgreSqlPdoDatabase extends QPdoDatabase
{
    ...
    public function InsertOrUpdate($strTable, $mixColumnsAndValuesArray, $strPKNames = null)
    {
        ...
        $strUpdateSql = sprintf('UPDATE %s SET %s WHERE %s', $strTable, $strUpdateStatement, $strMatchCondition);
        $strInsertSql = sprintf('INSERT INTO (%s) %s SELECT %s WHERE NOT EXISTS (SELECT 1 FROM %s WHERE %s)', $strTable, implode(', ', $strColumns), implode(', ', array_values($strEscapedArray)), $strTable, $strMatchCondition);
