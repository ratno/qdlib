...
class QSqLite3PdoDatabase extends QPdoDatabase
{
    ...
    public function InsertOrUpdate($strTable, $mixColumnsAndValuesArray, $strPKNames = null)
    {
        ...
        $strSql = sprintf('INSERT OR REPLACE INTO %s%s%s (%s) VALUES (%s)', $this->EscapeIdentifierBegin, $strTable, $this->EscapeIdentifierEnd, implode(', ', array_keys($strEscapedArray)), implode(', ', array_values($strEscapedArray))
        );
    ...
    {
        $strQuery = sprintf('PRAGMA table_info (%s%s%s)', $this->strEscapeIdentifierBeginInternal, $strTableName, $this->strEscapeIdentifierEndInternal);
    ...
    public function GetIndexesForTable($strTableName)
    {
        ...
        $objResult = $this->Query(sprintf(
            'PRAGMA index_list (%s%s%s)', $this->strEscapeIdentifierBeginInternal, $strTableName, $this->strEscapeIdentifierEndInternal));
    ...
    public function GetForeignKeysForTable($strTableName)
    {
        ...
        $strQuery = sprintf('PRAGMA foreign_key_list (%s%s%s)', $this->strEscapeIdentifierBeginInternal, $strTableName, $this->strEscapeIdentifierEndInternal);
