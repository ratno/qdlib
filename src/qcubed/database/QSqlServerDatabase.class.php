    ...
    public function SqlLimitVariablePrefix($strLimitInfo)
    {
        ...
        if (strlen($strLimitInfo)) {
            ...
            if (count($strArray) == 2) {
                ...
                return sprintf(
                    'TOP %s QCODO_OFFSET<%s>', ($strArray[0] + $strArray[1]), $strArray[0]);
...
//			$objResult = $this->Query("SELECT name FROM sysobjects WHERE xtype='U' ORDER BY name ASC");
    ...
    public function GetForeignKeysForTable($strTableName)
    {
        ...
        $strQuery = sprintf('
				...
				WHERE
				    FK.TABLE_NAME = %s
				ORDER BY 
				    constraint_name', $this->SqlVariable($strTableName));
        $objResult = $this->Query($strQuery);
            ...
            if ($strKeyName != $objRow->GetColumn('constraint_name')) {
                if ($strKeyName) {
                    $objForeignKey = new QDatabaseForeignKey(
                        $strKeyName, $strColumnNameArray, $strReferenceTableName, $strReferenceColumnNameArray);
    ...
    protected function ExecuteQuery($strQuery)
    {
        ...
        if (($intPosition = strpos($strQuery, 'QCODO_OFFSET<')) !== false) {
            ...
            $intOffset = QType::Cast(substr($strQuery, $intPosition + 13 /* len of QCODO_OFFSET< */, $intEndPosition - $intPosition - 13), QType::Integer);
            ...
            if (count($intIndIdArray) > 0) {
                ...
                $strQuery = sprintf('
						...
						WHERE
							...
							sysindexes.id = %s
					', implode(',', $intIndIdArray), $objDb->SqlVariable($this->strName), $intTableId, $intTableId, $intTableId);

                $objResult = $objDb->Query($strQuery);

                while ($objRow = $objResult->FetchRow()) {
                    $strQuery = sprintf("SELECT indexproperty(%s, %s, 'IsUnique')", $intTableId, $objDb->SqlVariable($objRow[0]));
