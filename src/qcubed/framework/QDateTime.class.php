...
class QDateTime extends DateTime
{
    ...
    const FormatDisplayTime = 'hhhh:mm';
    ...
    public function __construct($mixValue = null, DateTimeZone $objTimeZone = null, $intType = QDateTime::UnknownType, $strInputFormat = null)
    {
        if ($intType) {
            if ($objTimeZone) {
        ...
        if ($mixValue instanceof QDateTime) {
            ...
            // Parse the Value string --> default qc value string parsing
        } else {
            $strTimeISO8601 = null;
            $blnValid = false;
