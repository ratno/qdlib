...
abstract class QApplicationBase extends QBaseClass
{
     ...
     */
    public static $DefaultCrossScriptingMode = 'Legacy';
    /**
     ...
     */
    public static $DocumentRoot; //QCrossScripting::Legacy;
    /**
    ...
    public static function Initialize()
    {
        ...
        if (array_key_exists('PATH_INFO', $_SERVER)) {
            QApplication::$PathInfo = rawurlencode(trim($_SERVER['PATH_INFO']));
        } elseif (array_key_exists('ORIG_PATH_INFO', $_SERVER)) {
            QApplication::$PathInfo = rawurlencode(trim($_SERVER['ORIG_PATH_INFO']));
        } else {
            QApplication::$PathInfo = null;
        }
        if (!is_empty(QApplication::$PathInfo)) {
        ...
        // Preload Class Files
        if (is_array(QApplication::$PreloadedClassFile)) {
        ...
        for ($intIndex = 0; $intIndex <= $intMaxIndex; $intIndex++) {
            ...
            if (defined($strConstantName)) {
                ...
                if (!class_exists($strDatabaseType)) {
                    $strDatabaseAdapter = sprintf('%s/database/%s.class.php', dirname(dirname(__FILE__)), $strDatabaseType);
    ...
    public static function Autoload($strClassName)
    {
        if (isset(QApplication::$ClassFile[strtolower($strClassName)])) {
            ...
            return true;
        } else if (file_exists($strFilePath = sprintf('%s/%s.class.php', dirname(dirname(dirname(__FILE__))), $strClassName))) {
            require_once($strFilePath);
            return true;
        } else if (file_exists($strFilePath = sprintf('%s/controls/%s.class.php', dirname(dirname(__FILE__)), $strClassName))) {
            require_once($strFilePath);
            return true;
        }
    ...
    public static function GenerateQueryString($arr = null)
    {
        ...
        if (count($arr)) {
            ...
            foreach ($arr as $strKey => $mixValue)
                $strToReturn .= QApplication::GenerateQueryStringHelper(rawurlencode($strKey), $mixValue);
