...
abstract class QDatabaseBase extends QBaseClass
{
    ...
    public final function Query($strQuery)
    {
        ...
        if ($this->blnEnableProfiling) {
            ...
            QFirebug::info($strQuery, sprintf("%s::%s in %.3f ms", __CLASS__, __FUNCTION__, ($dblQueryTime * 1000)));
        }
    ...
    public final function NonQuery($strNonQuery)
    {
        ...
        if ($this->blnEnableProfiling) {
            ...
            QFirebug::info($strNonQuery, sprintf("%s::%s in %.3f ms", __CLASS__, __FUNCTION__, ($dblQueryTime * 1000)));
        }
    ...
    {
        if ($this->blnEnableProfiling) {
            printf('<form method="post" id="frmDbProfile%s" action="%s/error/db_profiling.php"><div>', $this->intDatabaseIndex, __BASEURL__);
    ...
    {
        $out = "";
        foreach ($this->strProfileArray as $profileItem) {
            $out .= "----------------------------------------------\n";
            $out .= "File: " . $profileItem['objBacktrace']['file'] . "\n";
            $out .= "Line: " . $profileItem['objBacktrace']['line'] . "\n";
            $out .= "Func: " . $profileItem['objBacktrace']['class'] . $profileItem['objBacktrace']['type'] . $profileItem['objBacktrace']['function'] . "\n";
            $out .= "Process Time: " . ($profileItem['dblTimeInfo'] * 1000) . " sekon\n";
            $out .= "Query: " . $profileItem['strQuery'] . "\n\n";
        }
        $out .= "----------------------------------------------\n";
        echo $out;
    }
