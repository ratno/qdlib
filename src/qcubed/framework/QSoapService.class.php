...
class QSoapParameter extends QBaseClass
{
        ...
        try {
            switch ($strName) {
                ...
                case 'Type':
                    return $this->strType;
                case 'Array':
                    return $this->blnArray;
                case 'Reference':
                    return $this->blnReference;
                default:
    ...
    public function __set($strName, $mixValue)
    {
        try {
            switch ($strName) {
                ...
                case 'Array':
                    return ($this->blnArray = QType::Cast($mixValue, QType::Boolean));
                case 'Reference':
                    return ($this->blnReference = QType::Cast($mixValue, QType::Boolean));
                default:
    ...
    {
        return sprintf('%s%s %s$%s', $this->strType, ($this->blnArray) ? '[]' : '', ($this->blnReference) ? '&' : '', $this->strName);
    }

    public function DisplayReturn()
    {
        return sprintf('%s%s', $this->strType, ($this->blnArray) ? '[]' : '');
    }
        ...
        if ($this->IsObject()) {
            ...
            if ($this->blnArray)
                return sprintf('$dttArray = array(); foreach(%s as $strDate) { ' .
                    'array_push($dttArray, new QDateTime($strDate)); } %s = $dttArray; ', $strArgumentName, $strArgumentName);
            else
...
class QSoapMethod extends QBaseClass
{
    ...
    public function GetWsdlPortTypeOperation()
    {
        return sprintf('<operation name="%s">' .
            ...
            '</operation>', $this->strName, $this->strName, $this->strName);
    }

    public function GetWsdlBindingOperation($strNamespace)
    {
        $strSoapBody = sprintf('<soap:body use="encoded" namespace="%s/%s" encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"/>', $strNamespace, $this->strName);
        return sprintf('<operation name="%s"><soap:operation soapAction="%s/%s"/>' .
            '<input>%s</input>' .
            '<output>%s</output></operation>', $this->strName, $strNamespace, $this->strName, $strSoapBody, $strSoapBody);
    }
        ...
        try {
            switch ($strName) {
                ...
                case 'ReturnParameter':
                    return $this->objReturnParameter;
                default:
                    return parent::__get($strName);
            }
        } catch (QCallerException $objExc) {
        ...
        try {
            switch ($strName) {
                case 'Name':
                    return ($this->strName = QType::Cast($mixValue, QType::String));
                case 'ReturnParameter':
                    return ($this->objReturnParameter = QType::Cast($mixValue, 'QSoapParameter'));
                default:
                    return parent::__set($strName, $mixValue);
            }
        } catch (QCallerException $objExc) {
...
class QSoapService extends QBaseClass
{
    ...
    public static $DefaultNamespace = 'http://knoqdown.com';
    ...
    public static function AlterComplexTypesArrayForArrayType($strArrayTypeName, $strType, &$strComplexTypesArray)
    {
        if (!array_key_exists($strArrayTypeName, $strComplexTypesArray))
            $strComplexTypesArray[$strArrayTypeName] = sprintf(
                ...
                '</restriction></complexContent></complexType>', $strArrayTypeName, $strType);
    }
    ...
    public function GetClassWrapper()
    {
        ...
        foreach ($this->objMethodArray as $objMethod) {
            ...
            foreach ($objMethod->GetAllParameters() as $objParameter) {
                $strParameterDefinition = sprintf('%s %s$%s', ($objParameter->Type == QType::DateTime) ? $objParameter->Type : '', ($objParameter->Reference) ? '&' : '', $objParameter->Name
                );
    ...
    protected function GetDisco()
    {
        ...
        $strToReturn .= sprintf('<soap address="%s" xmlns:q1="%s" binding="q1:%sSoap" xmlns="http://schemas.xmlsoap.org/disco/soap/" />', $strUrl, $this->strNamespace, $this->strClassName);
    ...
    {
        return sprintf('http%s://%s%s', (array_key_exists('HTTPS', $_SERVER) && $_SERVER['HTTPS']) ? 's' : '', $_SERVER['HTTP_HOST'], QApplication::$ScriptName);
    }
    ...
    protected function GetWsdl()
    {
        $strToReturn = sprintf('<definitions name="%s" ' .
            ...
            'xmlns:xsd1="%s">', $this->strClassName, $this->strNamespace, $this->strNamespace, $this->strNamespace);
    ...
    public function GetWsdlTypes($strComplexTypesArray)
    {
        $strToReturn = sprintf('<types><schema xmlns="http://www.w3.org/2001/XMLSchema" targetNamespace="%s" ' .
            'xmlns:wsdl="http://schemas.xmlsoap.org/wsdl/" xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/">', $this->strNamespace);
    ...
    protected function SetupSoapMethods(ReflectionClass $objReflection)
    {
        ...
        if ($objReflectionMethods)
            foreach ($objReflectionMethods as $objReflectionMethod) {
                if ($objReflectionMethod->isPublic() && !$objReflectionMethod->isAbstract() &&
                    ...
                    if ($strComments) {
                        ...
                        if ($objParameters)
                            foreach ($objParameters as $objParameter) {
                                $blnArray = false;
