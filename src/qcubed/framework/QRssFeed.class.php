...
//		protected $strSkipDays;
    ...
    public function __construct($strTitle, $strLink, $strDescription)
    {
        ...
        $this->strGenerator = 'QD Development Framework ' . QDF_VERSION;
    }
    ...
    public function __get($strName)
    {
        try {
            switch ($strName) {
                ...
                case 'ManagingEditor':
                    return $this->strManagingEditor;
                case 'WebMaster':
                    return $this->strWebMaster;
                case 'PubDate':
                    return $this->dttPubDate;
                case 'LastBuildDate':
                    return $this->dttLastBuildDate;
                case 'Generator':
    ...
    public function __set($strName, $mixValue)
    {
        try {
            switch ($strName) {
                ...
                case 'ManagingEditor':
                    return ($this->strManagingEditor = QType::Cast($mixValue, QType::String));
                case 'WebMaster':
                    return ($this->strWebMaster = QType::Cast($mixValue, QType::String));
                case 'PubDate':
                    return ($this->dttPubDate = QType::Cast($mixValue, QType::DateTime));
                case 'LastBuildDate':
                    return ($this->dttLastBuildDate = QType::Cast($mixValue, QType::DateTime));
                case 'Generator':
                    return ($this->strGenerator = QType::Cast($mixValue, QType::String));
                case 'Docs':
                    return ($this->strDocs = QType::Cast($mixValue, QType::String));
                case 'Ttl':
                    return ($this->strTtl = QType::Cast($mixValue, QType::String));
                case 'Image':
                    return ($this->objImage = QType::Cast($mixValue, 'QRssImage'));
                default:
    ...
    public function __set($strName, $mixValue)
    {
        try {
            switch ($strName) {
                ...
                case 'GuidPermaLink':
                    return ($this->blnGuidPermaLink = QType::Cast($mixValue, QType::Boolean));
                case 'PubDate':
                    return ($this->dttPubDate = QType::Cast($mixValue, QType::DateTime));
                default:
                    return parent::__set($strName, $mixValue);
