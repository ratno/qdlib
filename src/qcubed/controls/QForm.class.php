...
abstract class QForm extends QFormBase
{
    ...
    protected $blnFilter = false;
    protected $arrFilter = array();
    protected $arrColumns = array();

    protected function AddFilter($mixData)
    {
        if (is_array($mixData) && count($mixData) > 0) {
            foreach ($mixData as $item) {
                $this->arrFilter[] = $this->cekFilter($item);
            }
        } else {
            if (!is_null($mixData) && !empty($mixData) && $mixData != "") { // mix data through check
                $this->arrFilter[] = $this->cekFilter($mixData);
            }
        }
        $this->blnFilter = (count($this->arrFilter) > 0) ? true : false;
    }

    protected function CekFilter($mixData)
    {
        if ($mixData instanceof QQNode) {
            return $mixData->_PropertyName;
        } else {
            return $mixData;
        }
    }
