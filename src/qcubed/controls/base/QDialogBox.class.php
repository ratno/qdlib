...
class QDialogBox extends QPanel
{
    ...
    public function GetShowDialogJavaScript()
    {
        ...
        $strOptions .= ', open: function() { $(this).parent().appendTo($("form:first")); }';

        return sprintf('$(qc.getW("%s")).dialog({%s}).dialog("open");', $this->strControlId, $strOptions, $this->strControlId);
    }
    ...
    {
        return sprintf('var $dlg = $(qc.getW("%s")); if($dlg.is(":ui-dialog")) { $dlg.dialog("close"); }', $this->strControlId);
    }
