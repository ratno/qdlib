...
class QRadioButtonList extends QListControl
{
    ...
    public function GetControlHtml()
    {
        ...
        if ($this->intButtonMode == self::ButtonModeSet) {
            $strToReturn = sprintf('<div id="%s" %s%s%s%s%s>', $this->strControlId, $strAccessKey, $strToolTip, $strCssClass, $strStyle, $strCustomAttributes) . "\n";
    ...
    protected function GetItemHtml($objItem, $intIndex, $strActions, $strTabIndex)
    {
        ...
        $strLabel = sprintf('<label for="%s">%s</label>', $strIndexedId, ($this->blnHtmlEntities) ? QApplication::HtmlEntities($objItem->Name) : $objItem->Name
        );

        $strInput = sprintf('<input id="%s" name="%s" value="%s" type="radio" %s%s%s%s%s />', $strIndexedId, $this->strControlId, $intIndex, ($this->blnEnabled) ? '' : 'disabled="disabled"', ($objItem->Selected) ? 'checked="checked"' : '', $objStyle->GetAttributes(), $strActions, $strTabIndex
        );
