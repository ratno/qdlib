 ...
 * @property string $CustomTitle Custom Title For Page
 * @property string $CustomJs Custom Javascript For Page
 * @property string $CustomCss Custom CSS For Page
 * @property string $GlobalLayout Page Layout
 * @property array $ArrayCustom
 */
abstract class QFormBase extends QPage
{
    ...
    /**
     * This exists to prevent inadverant "New" --> change from protected to public by ratno, 2014-07-17
     */
    ...
    {
        performance(1, __CLASS__, __FUNCTION__);
        ...
        performance(1, __CLASS__, __FUNCTION__, "Get Form From POST data");
        $objClass = null;
        ...
        performance(2, __CLASS__, __FUNCTION__, "Get Form From POST data");

        if ($objClass) {
            // Globalize
            $_FORM = $objClass;

            // Call Page Set Information
            performance(1, __CLASS__, __FUNCTION__, "SetInformation");
            $objClass->SetInformation($strFormId);
            performance(2, __CLASS__, __FUNCTION__, "SetInformation");
            ...
            if ($objClass->strCallType == QCallType::Ajax)
                ...
                $param = array();
                parse_str(rawurldecode($_POST['Qform__FormParameter']), $param);
                $_POST['Qform__FormParameter'] = $param['Qform__FormParameter'];
            }

            // Iterate through all the control modifications
            performance(1, __CLASS__, __FUNCTION__, "Iterate Control Modification");
            ...
            performance(2, __CLASS__, __FUNCTION__, "Iterate Control Modification");

            // Clear the RenderedCheckableControlArray
            performance(1, __CLASS__, __FUNCTION__, "Clear Checkable control");
            ...
            performance(2, __CLASS__, __FUNCTION__, "Clear Checkable control");

            // Iterate through all the controls
            performance(1, __CLASS__, __FUNCTION__, "Iterate Controls");
            ...
            performance(2, __CLASS__, __FUNCTION__, "Iterate Controls");

            // Trigger Run Event (if applicable)
            performance(1, __CLASS__, __FUNCTION__, "Form_Run");
            $objClass->Form_Run();
            performance(2, __CLASS__, __FUNCTION__, "Form_Run");

            // Trigger Load Event (if applicable)
            performance(1, __CLASS__, __FUNCTION__, "Form_Load");
            $objClass->Form_Load();
            performance(2, __CLASS__, __FUNCTION__, "Form_Load");

            // Trigger a triggered control's Server- or Ajax- action (e.g. PHP method) here (if applicable)
            performance(1, __CLASS__, __FUNCTION__, "TriggerActions");
            $objClass->TriggerActions();
            performance(2, __CLASS__, __FUNCTION__, "TriggerActions");
        } else {
            ...
            performance(1, __CLASS__, __FUNCTION__, "Page_Create");
            $objClass->Page_Create();
            performance(2, __CLASS__, __FUNCTION__, "Page_Create");

            // Trigger Create Event (if applicable)
            performance(1, __CLASS__, __FUNCTION__, "Form_Create");
            $objClass->Form_Create();
            performance(2, __CLASS__, __FUNCTION__, "Form_Create");
        }

        // Trigger PreRender Event (if applicable)
        performance(1, __CLASS__, __FUNCTION__, "Form_PreRender");
        $objClass->Form_PreRender();
        performance(2, __CLASS__, __FUNCTION__, "Form_PreRender");

        // Render the Page
        switch ($objClass->strCallType) {
            case QCallType::Ajax:
                // Must use AJAX-based renderer
                performance(1, __CLASS__, __FUNCTION__, "RenderAjax");
                $objClass->RenderAjax();
                performance(2, __CLASS__, __FUNCTION__, "RenderAjax");
                break;
            ...
            case '':
                ...
                performance(1, __CLASS__, __FUNCTION__, "Render");
                $objClass->Render();
                performance(2, __CLASS__, __FUNCTION__, "Render");
                break;
        ...
        performance(1, __CLASS__, __FUNCTION__, "Form_Exit");
        $objClass->Form_Exit();
        performance(2, __CLASS__, __FUNCTION__, "Form_Exit");
        performance(2, __CLASS__, __FUNCTION__);
        return $objClass;
    }
...
//					$strToReturn .= $objControl->RenderAjax(false) . "\r\n";
        ...
        if (trim($strCommands))
            $strCommands = '<command>' . \JShrink\Minifier::minify(QString::XmlEscape(trim($strCommands))) . '</command>';
        ...
        // add flash messages
        $strToReturn .= '<flashmessages>';
        $strToReturn .= QString::XmlEscape($this->flash());
        $strToReturn .= '</flashmessages>';
...
//    } else
        ...
        // Setup Rendered HTML
        $strToReturn .= sprintf('<form method="post" id="%s" action="%s"%s>', $this->strFormId, htmlentities(complete_url(QApplication::$RequestUri)), $strFormAttributes);
        $strToReturn .= "\r\n";
    ...
    public function GetCssFileUri($strFile)
    {
        if ((strpos($strFile, "http") === 0) || (strpos($strFile, "https") === 0))
            return $strFile;
        if (strpos($strFile, "/") === 0)
            return __BASEURL__ . $strFile;
        return __CSS_URL__ . '/' . $strFile;
    }
...
//    $strJavaScriptArray = $this->ProcessJavaScriptList(__JQUERY_BASE__ . ', ' . __JQUERY_EFFECTS__ . ',jquery/jquery.ajaxq-0.0.1.js,' . __QCUBED_JS_CORE__);
        ...
        $strEndScript = sprintf('qc.baseDir = "%s"; ', __BASEURL__) . $strEndScript;
        $strEndScript = sprintf('qc.jsAssets = "%s"; ', __JS_URL__) . $strEndScript;
        $strEndScript = sprintf('qc.phpAssets = "%s"; ', __BASEURL__) . $strEndScript;
        $strEndScript = sprintf('qc.cssAssets = "%s"; ', __CSS_URL__) . $strEndScript;
        $strEndScript = sprintf('qc.imageAssets = "%s"; ', __IMAGES_URL__) . $strEndScript;


        // Create Final EndScript Script
        $strEndScript = sprintf('<script type="text/javascript">$(document).ready(function() { %s; });</script>', \JShrink\Minifier::minify($strEndScript));
