...
abstract class QDataGridBase extends QPaginatedControl
{
    ...
    /** header * */
    protected $intHeaderRowNum = 1;
    protected $arrHeader;
    protected $arrHeaderCells;
    protected $arrRestructure;
    protected $arrHeaderDataAttributes;
    ...
    public function RestructureHeader(array $arrRestructureHeader)
    {
        if (is_array($arrRestructureHeader) && count($arrRestructureHeader) > 0) {
            $this->arrRestructure = $arrRestructureHeader;
        }
    }

    public function SetHeaderDataAttributes(array $arrHeaderDataAttributes)
    {
        if (is_array($arrHeaderDataAttributes) && count($arrHeaderDataAttributes) > 0) {
            $this->arrHeaderDataAttributes = $arrHeaderDataAttributes;
        }
    }

    public function SetSumColumns($colName)
    {
        foreach (func_get_args() as $strSumColumns) {
            $this->arrSum[$strSumColumns] = array();
        }
    }
    ...
    protected function GetHeaderRowHtml()
    {
        ...
        $max_indentation = 1;
        // PHP_EOL in case we're running on Windows --> ga valid pakai "\n" aja
        $lines = explode("\n", print_r($this->arrRestructure, true));
        foreach ($lines as $line) {
            $indentation = (strlen($line) - strlen(ltrim($line))) / 4;
            $max_indentation = max($max_indentation, $indentation);
        }
        $this->intHeaderRowNum = ceil(($max_indentation - 1) / 2) + 1;

        $this->arrHeader = array();
        $this->arrHeaderCells = array();
        $intColumnIndex = 0;
        if ($this->objColumnArray)
            foreach ($this->objColumnArray as $objColumn) {
                $this->InitSumColumn($objColumn->ColName);
                if ($objColumn->OrderByClause) {
                    ...
                    $this->arrHeader[str_replace(" ", "", $objColumn->ColName ? $objColumn->ColName : $objColumn->Name)] = sprintf("    <a id=\"%s\" href=\"%s\" %s%s>%s</a>", $this->ControlId . "_col_" . $this->strActionParameter, QApplication::$RequestUri, $this->prxDatagridSorting->RenderAsEvents($this->strActionParameter, true, $this->ControlId . "_col_" . $this->strActionParameter, false), $this->objHeaderLinkStyle->GetAttributes(), $strName);
                } else {
                    if ($objColumn instanceof QCheckBoxColumn) {
                        $this->arrHeader[] = sprintf("    %s", $objColumn->Name);
                    } else {
                        $this->arrHeader[str_replace(" ", "", $objColumn->ColName ? $objColumn->ColName : $objColumn->Name)] = sprintf("    %s", $objColumn->Name);
                    }
                }
                $intColumnIndex++;
            }

        foreach ($this->arrHeader as $key => $mixHeaderItem) {
            $row = 0;
            if (array_key_exists($key, $this->arrHeader)) {
                if (is_array($this->arrRestructure) && array_key_exists($key, $this->arrRestructure)) {
                    $this->MergeHeader($this->arrRestructure[$key], $row);
                } else {
                    if ($this->CheckSumColumn($key)) {
                        $class = " class='sum'";
                    } else {
                        $class = "";
                    }
                    $strDataAttributes = "";
                    if (is_array($this->arrHeaderDataAttributes) && array_key_exists($key, $this->arrHeaderDataAttributes) && is_string($key) && is_array($this->arrHeaderDataAttributes[$key])) {
                        foreach ($this->arrHeaderDataAttributes[$key] as $strDataAttributeKey => $strDataAttributeValue) {
                            $strDataAttributes .= "data-$strDataAttributeKey='$strDataAttributeValue' ";
                        }
                    }
                    $this->arrHeaderCells[$row][] = sprintf("    <th%s rowspan='%s' %s %s>%s</th>\r\n", $class, $this->intHeaderRowNum, $this->objHeaderRowStyle->GetAttributes(), $strDataAttributes, $this->arrHeader[$key]);
                    unset($this->arrHeader[$key]);
                }
            } else {
                continue;
            }
        ...
        $strToReturn = "";
        for ($i = 0; $i < count($this->arrHeaderCells); $i++) {
            $strToReturn .= sprintf("  <tr %s>\r\n", $objHeaderStyle->GetAttributes());
            $strToReturn .= implode("\r\n", $this->arrHeaderCells[$i]);
            $strToReturn .= "  </tr>\r\n";
        }
    ...
    protected function InitSumColumn($strColName)
    {
        if ($this->CheckSumColumn($strColName)) {
            $this->arrSum[$strColName] = array();
        }
    }

    public function CheckSumColumn($strColName)
    {
        if (is_array($this->arrSum) && count($this->arrSum) > 0) {
            if (array_key_exists($strColName, $this->arrSum)) {
                return true;
            } else {
                return false;
            }
    ...
    protected function GetHeaderSortedHtml(QDataGridColumn $objColumn)
    {
        ...
        if ($this->intSortDirection == 0)
            $strToReturn .= sprintf(' <img src="%s/sort_arrow.png" width="7" height="7" border="0" alt="Sorted" />', __IMAGES_URL__);
        else
            $strToReturn .= sprintf(' <img src="%s/sort_arrow_reverse.png" width="7" height="7" border="0" alt="Reverse Sorted" />', __IMAGES_URL__);

        return $strToReturn;
    }

    protected function MergeHeader($data, $row)
    {
        $objHeaderStyle = $this->objRowStyle->ApplyOverride($this->objHeaderRowStyle);
        // $data schema is:
        // - title
        // - content (could be array of title and content too)
        $row++;
        $intColCounter = 0;
        foreach ($data['content'] as $mixItem) {
            if (is_array($mixItem) && array_key_exists("title", $mixItem)) {
                $intColCounter += $this->MergeHeader($mixItem, $row);
            } else {
                $span = $this->intHeaderRowNum - $row;
                if ($span) {
                    $rowspan = " rowspan='" . ($span) . "'";
                } else {
                    $rowspan = "";
                }
                if ($this->CheckSumColumn($mixItem)) {
                    $class = " class='sum'";
                } else {
                    $class = "";
                }
                ...
                $strDataAttributes = "";
                if (is_array($this->arrHeaderDataAttributes) && array_key_exists($mixItem, $this->arrHeaderDataAttributes) && is_string($mixItem) && is_array($this->arrHeaderDataAttributes[$mixItem])) {
                    foreach ($this->arrHeaderDataAttributes[$mixItem] as $strDataAttributeKey => $strDataAttributeValue) {
                        $strDataAttributes .= "data-$strDataAttributeKey='$strDataAttributeValue' ";
                    }
                }
                $this->arrHeaderCells[$row][] = sprintf("    <th%s%s %s %s>%s</th>", $rowspan, $class, $this->objHeaderRowStyle->GetAttributes(), $strDataAttributes, $this->arrHeader[$mixItem]);
                $intColCounter++;
                unset($this->arrHeader[$mixItem]);
            }
        }
        $row--;
        $this->arrHeaderCells[$row][] = sprintf("    <th colspan='%s' %s>%s</th>", $intColCounter, $this->objHeaderRowStyle->GetAttributes(), $data['title']);
    ...
    protected function GetFooterRowHtml()
    {
        ...
        if (is_array($this->arrSum) && count($this->arrSum) > 0) {
            ...
            if ($this->objColumnArray) {
                foreach ($this->objColumnArray as $objColumn) {
                    if (array_key_exists($objColumn->ColName, $this->arrSum)) {
                        $out .= '<th class="sum">';
                        $out .= number_format(array_sum($this->arrSum[$objColumn->ColName]), 0, ",", ".");
                    } else {
                        $out .= '<th>';
                        $out .= "&nbsp;";
                    }
    ...
    protected function GetDataGridRowHtml($objObject)
    {
        ...
        foreach ($this->objColumnArray as $objColumn) {
            ...
            if ($this->CheckSumColumn($objColumn->ColName)) {
                $strColumnsHtml .= sprintf("    <td %s class='sum'>%s</td>\r\n", $objColumn->GetAttributes(), $strHtml);
            } else {
                $strColumnsHtml .= sprintf("    <td %s>%s</td>\r\n", $objColumn->GetAttributes(), $strHtml);
            }
            $this->SetSumValue($objColumn->ColName, $strHtml);
        }
    ...
    public static function ParseHtml($strHtml, $objControl, $objColumn, $objObject)
    {
        ...
        while (($intStartPosition = strpos($strHtml, '<?=', $intPosition)) !== false) {
            ...
            if ($strToken) {
                ...
                set_error_handler("QcodoHandleError", MY_ERROR_REPORTING);
            ...
            // render '-' on column when no data available, added by ratno @may 8, 2011
            if (empty($strEvaledToken) or is_null($strEvaledToken) or $strEvaledToken == "") {
                $strEvaledToken = "-";
            }
    ...
    public function SetSumValue($strColName, $intValue)
    {
        if ($this->CheckSumColumn($strColName)) {
            if (is_numeric($intValue) || is_bool($intValue)) {
                $this->arrSum[$strColName][] = $intValue;
            }
        }
    }
