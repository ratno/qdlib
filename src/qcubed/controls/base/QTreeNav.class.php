...
class QTreeNav extends QControl
{
    ...
    protected function GetItemHtml($objItem)
    {
        ...
        $strCommand = sprintf('onclick="qc.pA(\'%s\', \'%s\', \'QChangeEvent\', \'%s\', \'%s\')"', $this->objForm->FormId, $this->strControlId, $strItemId, $this->objForm->DefaultWaitIcon->ControlId
        );
        if ($this->blnAllowHover)
            $strLabelHtml = sprintf('<span id="%s_label" class="%s" %s>%s</span>', $strItemId, ($objItem->Selected) ? $this->strItemSelectedCssStyle : $this->strItemCssStyle, $strItemId, ($objItem->Selected) ? $this->strItemSelectedHoverCssStyle : $this->strItemHoverCssStyle, $strItemId, ($objItem->Selected) ? $this->strItemSelectedCssStyle : $this->strItemCssStyle, $strCommand, $objItem->Name);
        else
            $strLabelHtml = sprintf('<span id="%s_label" class="%s" %s>%s</span>', $strItemId, ($objItem->Selected) ? $this->strItemSelectedCssStyle : $this->strItemCssStyle, $strCommand, $objItem->Name);
    ...
    public function GetControlHtml()
    {
        ...
        return sprintf('<div id="%s" %s%s>%s</div>', $this->strControlId, $strAttributes, $strStyles, $strItemHtml);
    }
