...
abstract class QAction extends QBaseClass
{
    ...
    public static function RenderActions(QControl $objControl, $strEventName, $objActions)
    {
        ...
        if ($objActions && count($objActions))
            foreach ($objActions as $objAction) {
                ...
                if ($objAction->objEvent->Delay > 0) {
                    $strCode = sprintf(" qcubed.setTimeout('%s', '%s', %s);", $objControl->ControlId, addslashes($objAction->RenderScript($objControl)), $objAction->objEvent->Delay);
                } else {
        ...
        if (strlen($strToReturn)) {
            if ($strJqUiProperty) {
                return sprintf('$("#%s").%s("option", {%s: function(event, ui){
								%s
								}});
                ...
                if ($objControl->TargetControlId) {
                    return sprintf('$("#%s").on("%s", function(event, ui){
									%s
									});
...
class QServerAction extends QAction
{
    ...
    {
        return sprintf("qc.pB('%s', '%s', '%s', %s);", $objControl->Form->FormId, $objControl->ControlId, get_class($this->objEvent), $this->getActionParameter($objControl));
    }
...
class QAjaxAction extends QAction
{
    ...
    public function RenderScript(QControl $objControl)
    {
        ...
        return sprintf("qc.pA('%s', '%s', '%s#%s', %s, '%s');", $objControl->Form->FormId, $objControl->ControlId, get_class($this->objEvent), $this->strId, $this->getActionParameter($objControl), $strWaitIconControlId);
    }
...
class QToggleDisplayAction extends QAction
{
    ...
    public function RenderScript(QControl $objControl)
    {
        ...
        return sprintf("qc.getW('%s').toggleDisplay('%s');", $this->strControlId, $strShowOrHide);
    }
...
class QCssAction extends QAction
{
    ...
    public function RenderScript(QControl $objControl)
    {
        if ($this->strControlId == null)
            $this->strControlId = $objControl->ControlId;
        // Specified a Temporary Css Class to use?
        return sprintf('$("#%s").css("%s", "%s"); ', $this->strControlId, $this->strCssProperty, $this->strCssValue);
    }
