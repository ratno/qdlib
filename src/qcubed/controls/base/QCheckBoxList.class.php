...
class QCheckBoxList extends QListControl
{
    ...
    protected function GetItemHtml($objItem, $intIndex, $strActions, $strTabIndex)
    {
        ...
        $strInput = sprintf('<input id="%s" name="%s" type="checkbox" %s%s%s%s%s />', $strIndexedId, $this->strControlId . '[' . $intIndex . ']', ($this->blnEnabled) ? '' : 'disabled="disabled"', ($objItem->Selected) ? 'checked="checked"' : '', $objStyle->GetAttributes(), $strActions, $strTabIndex
        );
