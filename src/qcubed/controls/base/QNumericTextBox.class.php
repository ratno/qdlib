 ...
 * @property string $Mask            Masking data input, default: 000.000.000.000.000
 */
abstract class QNumericTextBox extends QTextBox
{
    ...
    /** @var string Text to set numeric masking */
    protected $strMask = "99.999";
    ...
    public function __construct($objParentObject, $strControlId = null)
    {
        ...
        $this->AddJavascriptFile("inputmask.min.js");
        $this->AddJavascriptFile("jquery.inputmask.min.js");
//    $this->AddJavascriptFile("jquery.maskMoney.min.js");
    }
    ...
    public function GetEndScript()
    {
        if (!$this->blnVisible) {
            $strCtlScript = '';
        } elseif (!$this->blnEnabled) {
            $strCtlScript = '';
        } else {
            $strCtlScript = sprintf("jQuery('#%s').inputmask('%s',{ numericInput: true, radixPoint: ',', groupSeparator:'.' });",
                $this->strControlId,
                $this->strMask
            );
        }
        return $strCtlScript . parent::GetEndScript();
    }
    ...
    public function Validate()
    {
        if (parent::Validate()) {
            if ($this->strText != "") {
                try {
                    $this->strText = QType::Cast($this->Text, $this->strDataType);
    ...
    public function __get($strName)
    {
        switch ($strName) {
            ...
            case 'Mask':
                return $this->strMask;
            case 'Text':
                return getRealValueWithMasking($this->strText, $this->strMask);
    ...
    public function __set($strName, $mixValue)
    {
        ...
        switch ($strName) {
            ...
            case 'Text':
                try {
                    $strText = $mixValue;
                    $this->strText = getRealValueWithMasking($strText, $this->strMask);
                    break;
                } catch (QInvalidCastException $objExc) {
                    $objExc->IncrementOffset();
                    throw $objExc;
