...
abstract class QListBoxBase extends QListControl
{
    ...
    public function GetControlHtml()
    {
        ...
        $strToReturn = sprintf('<select name="%s%s" id="%s" %s%s>', $this->strControlId, ($this->strSelectionMode == QSelectionMode::Multiple) ? "[]" : "", $this->strControlId, $this->GetAttributes(), $strStyle);
    ...
    protected function GetItemHtml($objItem, $intIndex)
    {
        ...
        $strToReturn = sprintf('<option value="%s" %s%s>%s</option>', $intIndex, ($objItem->Selected) ? 'selected="selected"' : "", $objStyle->GetAttributes(), QApplication::HtmlEntities($objItem->Name)
        );
