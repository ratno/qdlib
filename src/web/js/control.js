...
/////////////////////////////
	...
	qcubed.registerControl = function(mixControl) {
		...
		objWrapper.registerClickPosition = function(objEvent) {			
			...
			$('#' + this.control.id + "_x").val(intX);
			$('#' + this.control.id + "_y").val(intY);
		};
		...
		objWrapper.select = function() {
			$('#' + this.control.id).select();
		};

		// Blink
		objWrapper.blink = function(strFromColor, strToColor) {		
			$('#' + this.control.id).css('background-color', '' + strFromColor);
			$('#' + this.control.id).animate({ backgroundColor: '' + strToColor }, 500)
		};
	}
